...
To generate a midi file of a composition, run `lein midi <infile> <outfile>`.
...
Most basically, an individual is penalized for the distance the first and last note are from the tonic (0). A phrase should start and end on the tonic in order to resolve properly.
...
In addition to the fitness criteria used for themes, the development section adds an award for repetitions of note durations and values from both themes in a similar fashion to the repetition reward used in themes. This encourages the development section to tie together with the themes.

#### Chord Fitness Criteria

Chords are generated by only creating a rhythm of the length of the phrase they are associated with. When the rhythm is generated, a major triad rooted at whatever note the associated melody happens to be playing is played. Thus, only the rhythmic criteria from the theme development are used for chords. The distribution of note durations is also skewed to encourage longer durations.
